...
WHERE student_id = '$stud_id' AND semester_id = '$sem_id' AND ay_id = '$ay_id'") or die(mysqli_error($conn));
    ...
    if ($result2 > 0) {
        header('location: all_forms_accSH.php?stud_id=' . $stud_id . '&glvl_id=' . $glvl_id);
    } else {
...
WHERE sy.student_id = '$stud_id' AND ay.academic_year = '$acad' AND sy.semester_id = '0'") or die(mysqli_error($conn));
...
$pdf->SetXY(5, 62);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(43, 5, 'SCHOLARSHIP DISCOUNT:', 0, 0);

$array = array("ACADEMIC:", "CULTURAL:", "ATHLETIC:", "SIBLING:", "EMPLOYEE:", "E.O.O.SCHLR.:", "GOVERNMENT ASSISTANCE:",
 "ESC", "NTUP", "WTUP", "NO VOUCHER", "_______________");
...
foreach ($array as $arrayvalue) {
    ...
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(5, 3, '', 0, 0);
    $pdf->Rect(5, $rectangle, 3, 3);

    $fontsize = 11;
    $tempFontSize = $fontsize;
    $cellwidth = 36;
    while ($pdf->GetStringWidth ($arrayvalue) > $cellwidth) {
        $pdf->SetFontSize($tempFontSize -= 0.1);
    }

    $pdf->Cell(38, 4, $arrayvalue, 0, 0);
...
while ($row1 = mysqli_fetch_array($assessed_info)) {
    ...
    $pdf->SetXY(48, 67); // start
    $pdf->Cell(15, 4, '', 1, 0, 'C');
    

    if ($row1['payment'] == 'cash') {

        $pdf->Cell(45, 4, 'Tuition Fee - '. $row1['payment'], 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['tuition_fee'], 2), 1, 1, 'C');

    } elseif ($row1['payment'] == 'trimestral') {

        $pdf->Cell(45, 4, 'Upon Enrollment - '. $row1['payment'], 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['upon_enrollment_tri'], 2), 1, 1, 'C');

    } else {

        $pdf->Cell(45, 4, 'Upon Enrollment - '. $row1['payment'], 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['upon_enrollment_quar'], 2), 1, 1, 'C');

    }
    $pdf->SetXY(48, 71);
    $pdf->Cell(15, 4, '', 1, 0, 'C');
    $pdf->Cell(45, 4, 'Discounts:', 1, 0, 'L');
    $pdf->Cell(25, 4, '', 1, 0, 'C');
    $pdf->Cell(27, 4, '', 1, 1, 'C');
    ...
    if ($row1['payment'] == 'trimestral') {

        $upon_enrollment = $row1['upon_enrollment_tri'];

    } else {
        
        $upon_enrollment = $row1['upon_enrollment_quar'];

    }

    $get_disc_id = mysqli_query($acc, "SELECT * FROM tbl_discounts WHERE ay_id= '$ay_id' ORDER BY discount_status,percent DESC") or die(mysqli_error($acc));

    while ($row2 = mysqli_fetch_array($get_disc_id)) { //boxes for discounts

        if (in_array($row2['disc_id'], $discount_array) == true) {

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, $row2['discount_desc'], 1, 0, 'C'); //description
    
        if ($row2['discount_status'] == 1) { //pag naka labas lang yung numbers

            $pdf->Cell(25, 4, '*', 1, 0, 'C');
            $pdf->Cell(27, 4, '', 1, 1, 'C'); // balance
                                                                    
        } else { // may value yung number

            if ($row2['percent'] == 1) {

                if ($row1['payment'] == 'cash') { //PAYMENT TYPES

                $percent_value = number_format(floor((($row2['discount'] / 100) * $tuition_fee) * 100)/ 100, 2, '.', ''); //this took forever king ina
                $tuition_fee = $tuition_fee - $percent_value;

                } else {
                    $percent_value = number_format(floor((($row2['discount'] / 100) * $cash_basis) * 100)/ 100, 2, '.', ''); //this took forever king ina
                    $cash_basis = $cash_basis - $percent_value;
                    $upon_enrollment = $upon_enrollment - $percent_value;
                
                }

                $pdf->Cell(25, 4, $row2['discount'].'% ('. number_format($percent_value, 2) .')', 1, 0, 'C');

            } else {

                if ($row1['payment'] == 'cash') {

                    $tuition_fee = $tuition_fee - $row2['discount'];

                } else {

                    $cash_basis = $cash_basis - $row2['discount'];
                    $upon_enrollment = $upon_enrollment - $row2['discount'];

                }
                
                

                $pdf->Cell(25, 4,number_format($row2['discount'], 2), 1, 0, 'C');
            ...
            if ($row1['payment'] == 'cash') { //total

                $total = $tuition_fee + $row1['lms'] + $row1['miscell_fee'] + $row1['instruct_mat'];
                ...
                $total = $upon_enrollment + $row1['first_tri'] + $row1['second_tri'];

            } else {

                

                $total = $upon_enrollment + $row1['first_quar'] + $row1['second_quar'] + $row1['third_quar'] + $row1['fourth_quar'];
    ...
    if ($row1['payment'] == 'cash') {
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Total tuition fee', 1, 0, 'L');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4,  number_format($tuition_fee, 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Miscellenous', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['miscell_fee'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Learning Management System', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['lms'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Instructional Materials', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['instruct_mat'], 2), 1, 1, 'C');
        $y2 += 4;
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Total upon enrollment:', 1, 0, 'L');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4,  number_format($upon_enrollment, 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'First Semester', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['first_tri'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Second Semester', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['second_tri'], 2), 1, 1, 'C');
        $y2 += 4;
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'First Quarter', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['first_quar'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Second Quarter', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['second_quar'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Third Quarter', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['third_quar'], 2), 1, 1, 'C');
        $y2 += 4;

        $pdf->SetXY(48, $y2);
        $pdf->Cell(15, 4, '', 1, 0, 'C');
        $pdf->Cell(45, 4, 'Fourth Quarter', 1, 0, 'C');
        $pdf->Cell(25, 4, '', 1, 0, 'C');
        $pdf->Cell(27, 4, number_format($row1['fourth_quar'], 2), 1, 1, 'C');
        $y2 += 4;
    ...
    for ($x = 0 ; $x <= $addboxes; $x ++) {
        ...
        if ($row1['payment'] == 'cash') {
            ...
            $upon_enrollment = $row1['upon_enrollment_quar'];
            $total = $upon_enrollment + $row1['first_quar'] + $row1['second_quar'] + $row1['third_quar'] + $row['fourth_quar'];
    ...
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(15, 4, '', 1, 0, 'C');
    $pdf->Cell(45, 4, 'TOTAL', 1, 0, 'C');
    $pdf->Cell(25, 4, '', 1, 0, 'C');
    $pdf->Cell(27, 4,  number_format($total, 2), 1, 1, 'C');
...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    if ($row1['payment'] == 'cash') {
        ...
        $pdf->Cell(55, 4, 'Tuition Fee - '. $row1['payment'] , 1, 0, 'C');
        $pdf->Cell(30, 4, '', 1, 0, 'C');
        $pdf->Cell(37, 4, number_format($row1['tuition_fee'], 2), 1, 1, 'C');

    }  else {

        $pdf->Cell(55, 4, 'Upon enrollment - '. $row1['payment'], 1, 0, 'C');
        $pdf->Cell(30, 4, '', 1, 0, 'C');
        $pdf->Cell(37, 4, number_format($upon_enrollment, 2), 1, 1, 'C');
    ...
    while ($row2 = mysqli_fetch_array($get_disc_id)) {

        if (in_array($row2['disc_id'], $discount_array) == true) {

        if ($row2['discount_status'] == 1) {
    ...
    if ($row1['payment'] == 'cash') {
        ...
        $pdf->Cell(20, 4, '', 1, 0, 'C');
        $pdf->Cell(55, 4, 'Tuition Fee w/ Discounts' , 1, 0, 'C');
        $pdf->Cell(30, 4, '', 1, 0, 'C');
        $pdf->Cell(37, 4, number_format($tuition_fee, 2), 1, 1, 'C');

    } else {

        $pdf->Cell(20, 4, '', 1, 0, 'C');
        $pdf->Cell(55, 4, 'Upon Enrollment w/ Discounts' , 1, 0, 'C');
        $pdf->Cell(30, 4, '', 1, 0, 'C');
        $pdf->Cell(37, 4, number_format($upon_enrollment, 2), 1, 1, 'C');
